...
// application.yml 파일에 등록된 time 컬럼의 데이터 타입, 포맷 조회 (retrieveTimeColumnInfo)
export const useTimeColumn = () => {
  return useQuery([`/api/v1/metas/time-column`], () => retrieveTimeColumn());
};
...
export const useMetaColumnIsResolutionInfo = (metaTblId: string, searchInfo: MetaColumnIsResolutionInfoSearchProps) => {
  return useQuery(
    ['/meta-column-is-resolution/info', metaTblId],
    () => retreiveMetaColumnIsResolutionInfo(metaTblId, searchInfo),
    { enabled: false, suspense: false }
  );
};
// Master Profile 선택된 메타테이블 id 값으로 메타컬럼테이블조회 meta_tbl_id 에 따라 조회(속성 조인키 조회)
export const useMetaColumnIsResolutionJoinkeyInfo = (
  metaTblId: string,
  searchInfo: MetaColumnIsResolutionInfoSearchProps
) => {
  return useQuery(
    ['/meta-column-is-resolution/info-joinkey', metaTblId],
    () => retreiveMetaColumnIsResolutionInfo(metaTblId, searchInfo),
    { enabled: false, suspense: false }
  );
};
